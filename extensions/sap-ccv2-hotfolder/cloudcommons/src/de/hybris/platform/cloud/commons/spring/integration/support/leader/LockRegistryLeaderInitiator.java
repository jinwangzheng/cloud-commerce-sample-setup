...
/**
 * <b>Note:</b> This is a copy of
 * {@link org.springframework.integration.support.leader.LockRegistryLeaderInitiator} with
 * bug fixes applied.  The majority of the code has been taken from spring integration v5
 * and has been adapted to work with v4.  Bug fixes are described in inline comments.
 *
 * Please also see
 * {@see https://stackoverflow.com/questions/53638213/spring-lockregistryleaderinitiator-making-excessive-calls-in-some-lock-registry}
 *
 * LockRegistryLeaderInitiator is component that initiates leader election based on holding a lock.
 * If the lock has the right properties (global with expiry), there will never be more than one leader,
 * but there may occasionally be no leader for short periods. If the lock has stronger
...
@SuppressWarnings({"squid:S1193", "squid:S134", "squid:S3776", "squid:S1312",
		"checkstyle:LocalVariableCouldBeFinal", "squid:S2221", "squid:MethodCyclomaticComplexity"})
public class LockRegistryLeaderInitiator implements SmartLifecycle, DisposableBean, ApplicationEventPublisherAware
{
	...
	private static final Log LOG = LogFactory.getLog(LockRegistryLeaderInitiator.class);
	 ...
	 */
	public LockRegistryLeaderInitiator(final LockRegistry locks)
	{
	...
	public LockRegistryLeaderInitiator(final LockRegistry locks, final Candidate candidate)
	{
	...
	public void setHeartBeatMillis(final long heartBeatMillis)
	{
		this.heartBeatMillis = heartBeatMillis;
	}

	public void setBusyWaitMillis(final long busyWaitMillis)
	{
	 ...
	 */
	public void setLeaderEventPublisher(final LeaderEventPublisher leaderEventPublisher)
	{
	...
	public void start()
	{
		...
		synchronized (this.lifecycleMonitor)
		{
			if (!this.running)
			{
				...
				LOG.debug("Started LeaderInitiator");
			}
		}
	}
	...
	public void stop()
	{
		synchronized (this.lifecycleMonitor)
		{
			if (this.running)
			{
				...
				this.future = null;
				LOG.debug("Stopped LeaderInitiator for " + getContext());
			}
		}
	}
	...
	protected class LeaderSelector implements Callable<Void>
	{
		...
		LeaderSelector(final String lockKey)
		{
		...
		public Void call()
		{
			try
			{
				while (isRunning())
				{
					try
					{
						...
						{
							LOG.debug("Acquiring the lock for " + this.context);
						}

						// THIS HAS BEEN MODIFIED TO ENSURE THAT ONLY A SINGLE
						// ATTEMPT TO ACQUIRE THE LOCK IS MADE, FOLLOWED BY A SLEEP
						// FOR busyWaitMillis.
					...
					catch (final Exception e)
					{
						if (this.locked)
						{
							...
							{
								LOG.debug("Could not unlock - treat as broken " + this.context +
						...
						if (e instanceof InterruptedException || Thread.currentThread().isInterrupted())
						{
							...
							if (isRunning())
							{
								LOG.warn("Restarting LeaderSelector for " + this.context + " because of error.", e);
						...
						else
						{
							...
							if (LOG.isDebugEnabled())
							{
								LOG.debug("Error acquiring the lock for " + this.context +
			...
			finally
			{
				...
				{
					LOG.debug("Could not unlock during stop for " + this.context
		...
		private void handleGranted() throws InterruptedException
		{
			...
			if (LockRegistryLeaderInitiator.this.leaderEventPublisher != null)
			{
				...
				{
					LOG.warn("Error publishing OnGranted event.", e);
				}
			}
		}
		...
		private void handleRevoked()
		{
			...
			if (LockRegistryLeaderInitiator.this.leaderEventPublisher != null)
			{
				...
				{
					LOG.warn("Error publishing OnRevoked event.", e);
				}
			}
		}
	}
	...
	private class LockContext implements Context
	{
		...
		public void yield()
		{
			if (LOG.isDebugEnabled())
			{
				LOG.debug("Yielding leadership from " + this);
			}
