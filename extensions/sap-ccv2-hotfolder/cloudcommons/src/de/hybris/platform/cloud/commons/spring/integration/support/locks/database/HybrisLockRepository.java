...
public class HybrisLockRepository implements SmartLifecycleRole, LockRepository, BeanNameAware
{
	...
	{
		final String currentRegion = getRegion();
		final int clusterId = getClusterId();
		LOG.warn("Unexpected code tries to delete expired locks in region [{}] for node [{}]",
			currentRegion, clusterId);
		synchronized (this.deletingMonitor)
		{
			final List<ApplicationResourceLockModel> locks = getApplicationResourceLockDao()
				.getLocks(currentRegion, clusterId);
			for (ApplicationResourceLockModel lock : locks)
			{
				try
				{
					deleteExpiredLocks(lock.getLockKey());
				} catch (final RuntimeException exception)
				{
					final String lockKey = lock.getLockKey();
					LOG.error("Unable to delete lock {} in region [{}] for node [{}] due to nested exception - {}",
						lockKey, currentRegion, clusterId, exception.getMessage(), exception);
				}
			}
		}
	}
	...
	@Override
	public boolean renew(final String key)
	{
		final String currentRegion = getRegion();
		final int clusterId = getClusterId();
		LOG.warn("Unexpected code tries to renew lock for key [{}] in region [{}] for node [{}]",
			key, currentRegion, clusterId);
		LOG.debug("Renewing lock for key [{}] in region [{}] for node [{}].", key, currentRegion, clusterId);
		try
		{
			modelService.enableTransactions();
			deleteExpiredLocks(key);
			try
			{
				final ApplicationResourceLockDao lockDao = getApplicationResourceLockDao();
				final ApplicationResourceLockModel lock = lockDao.getLock(key, getRegion(), getClusterId());

				if (lock == null)
				{
					LOG.debug("No lock found for key [{}] in region [{}] for node [{}]. Most likely it expired.",
						key, currentRegion, clusterId);
					return false;
				}
				else
				{
					...
					lock.setTimestamp(timestamp);
					logLockStatus(lock);
					final boolean saved = saveLock(lock);
					LOG.debug("Lock for key [{}] in region [{}] for node [{}] has been renewed with expireAt {}.",
						key, currentRegion, clusterId, timestamp);
					return saved;
				}
			}
			finally
		...
		{
			LOG.debug("Unable to renew record for lock for key [{}] in region [{}] for node [{}].",
				key, getRegion(), getClusterId(), ex);
			return false;
		}
	}
	...
	protected void deleteLock(final ApplicationResourceLockModel lock)
	{
		...
		{
			LOG.warn("Unable to delete detatched lock [{}].", lock);
		}
	}
