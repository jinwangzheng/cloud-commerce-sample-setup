...
public class HybrisAwareThreadPoolTaskExecutorTest
{
	...
	public void initialize_shouldCreateHybrisAwareThreadPoolExecutorCorrectly_LinkedQueue()
	{
		...
		assertThat(threadPoolExecutor).isEqualToComparingOnlyGivenFields(expected, "tenant", "corePoolSize", "maximumPoolSize", "threadFactory", "rejectedExecutionHandler");
		assertThat(threadPoolExecutor.getKeepAliveTime(TimeUnit.SECONDS)).isEqualTo(expected.getKeepAliveTime(TimeUnit.SECONDS));
		assertThat(threadPoolExecutor.allowsCoreThreadTimeOut()).isEqualTo(expected.allowsCoreThreadTimeOut());
	...
	@Test
	public void afterPropertiesSet_shouldSetupTenantAndInitializeExecutor()
	{
		assertThat(executor.isRunning()).isFalse();
