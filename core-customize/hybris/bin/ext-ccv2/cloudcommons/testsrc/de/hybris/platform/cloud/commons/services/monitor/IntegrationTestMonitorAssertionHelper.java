...
{
	private static final Logger LOG = getLogger(IntegrationTestMonitorAssertionHelper.class);
	...
	public void assertHistoryIsPresent(final MonitorHistoryData expectedHistory)
	{
		...
		waitUntil(Duration.ofSeconds(60), () -> {
			final List<MonitorHistoryDataModel> histories = monitorHistoryDataDao.find(params);
			final MonitorStatus expectedStatus = MonitorHistoryEnumHelper.getMonitorStatus(expectedHistory.getStatus());
			final MonitorStatus actualStatus = histories.stream()
				.findFirst()
				.map(MonitorHistoryDataModel::getStatus)
				.orElse(null);
			LOG.info("Waiting '{}' to be '{}'.", actualStatus, expectedStatus);
			return Objects.equals(actualStatus, expectedStatus);
		});
	...
	private void waitUntil(final Duration timeout, final Supplier<Boolean> condition)
	{
		final long until = System.currentTimeMillis() + timeout.toMillis();
