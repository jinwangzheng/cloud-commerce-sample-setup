...
public class BlobTransferredAspectTest
{
    ...
    public void setUp() throws Throwable
    {
        ...
        given(pjp.proceed()).willReturn(result);
		  ...
		  final BlobClient entry = mock(BlobClient.class);
		  given(entry.getProperties()).willReturn(properties);
		  given(entry.getProperties().getLastModified()).willReturn(new Date().toInstant().atOffset(ZoneOffset.of("+05:30")));
		  given(entry.getBlobName()).willReturn( PATH + "/" + FILE_NAME);

        when(session.get(PATH)).thenReturn(entry);

//        given(monitorService.begin(SystemArea.HOT_FOLDER, FILE_NAME)).willReturn(monitorHistory);
	...
	public void givenMethodFailed_thenShouldLogFailedStep_andIssueEndMonitor() throws Throwable
	{
		...
		given(pjp.proceed()).willThrow(ioException);
		...
		order.verify(monitorHistory).stepFailed(eq(Step.DOWNLOADED), isA(Date.class), isA(Date.class),
				eq(ioException), eq("Failed to read read blob [{}]"), eq(PATH));
