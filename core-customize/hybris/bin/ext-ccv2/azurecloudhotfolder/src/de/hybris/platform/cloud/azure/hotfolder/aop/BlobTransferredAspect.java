...
@SuppressWarnings({"squid:S00112", "squid:S2143"})
public class BlobTransferredAspect extends AbstractMonitoringAspect
{
	...
	public Object aroundRead(final ProceedingJoinPoint pjp, final String path) throws Throwable
	{
		...
		final BlobClient blob = getTargetOfMonitoringAspect(pjp, path);
	...
	private BlobClient getTargetOfMonitoringAspect(final ProceedingJoinPoint pjp, final String path) throws IOException
	{
		Assert.isTrue(pjp.getTarget() instanceof ExtendedSession, "pjp is not of type Extended Session");

		@SuppressWarnings("unchecked")
		final ExtendedSession<BlobClient> session = (ExtendedSession<BlobClient>) pjp.getTarget();
